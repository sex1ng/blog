...
$posts_per_page = 15;
$args           = ['paged' => $paged, 'posts_per_page' => $posts_per_page];
query_posts($args);
if (have_posts()) {
    $post_list = [];
    while (have_posts()) {
    ...
    <main class="ori-container columns <?php echo $main_class; ?> grid-md">
        <section class="timeline-list column <?php echo $post_list_class; ?>">
            ...
            <article <?php post_class('p-content'); ?> id="post-<?php the_ID(); ?>">
                ...
                <?php foreach ($categories_count = get_categories(['exclude' => 1]) as $item) : ?>
                    <h3 id="title-0">
                        <?php echo $item->cat_name ?>
                    </h3>
                    <ul class="sitemap-list">
                        <?php foreach (get_posts(array_merge($args, ['category' => $item->cat_ID])) as $post) : ?>
                            <p class="arch_box">
                                ...
                                <span class="arch_info">
                                    ...
                                    <?php echo get_comments_number($post->ID) ?>条评论&nbsp;&nbsp;&nbsp;
            ...
            <?php
            $unclassified = count(get_categories(['include' => 1]));
            $page_count   = ceil(($count - $unclassified) / (count($categories_count) * $posts_per_page));
            ?>
            <ul class="pagination">
                <?php if ($paged != 1) : ?>
                    <?php $last_url = home_url(add_query_arg(array('paged' => $paged - 1))) ?>
                    <li class="page-item">
                        <a class="last page-numbers" href="<?php echo $last_url ?>">
                ...
                <?php for ($i = 1; $i <= $page_count; $i++) {
                    ...
                    if ($current) {
                        ...
                        $href = home_url(add_query_arg(array('paged' => $i)));
                        echo '<a class="page-numbers" href="' . $href . '">' . $i . '</a>';
                    }
                    echo '</li>';
                } ?>
                <?php if ($paged < $page_count) : ?>
                    <?php $next_url = home_url(add_query_arg(array('paged' => $paged + 1))) ?>
                    <li class="page-item">
                        <a class="next page-numbers" href="<?php echo $next_url ?>">
                            下一页&nbsp;
            ...
            <?php //echo $pagination; 主题封装的分页插件总是会多一页。该问题暂未解决。 ?>
        </section>
